...
def test_the_picker_offers_colours_first_and_then_the_real_colormaps():
    """Two kinds of thing, and they are not interchangeable.

    A mask is a *region* -- "these voxels" -- and a ramp claims a magnitude it
    does not have, so a plain colour is the right answer for most overlays and
    the picker offers a classic swatch grid first. NiiVue paints with colormaps
    and nothing else, so the viewer turns a colour into one: a single hue ramping
    up from transparent, the same shape the package's own `red` and `violet` use.
    The document just records hex, because which of the two it is is all the file
    needs to know.

    Then the colormaps, and they are the shipped ones: greys for a scan, heat for
    activation, perceptual for a quantity that must not lie, the CT window
    presets, categorical for labels.
    """
    import json
    import re
    from pathlib import Path
    ...
    root = Path(__file__).resolve().parents[2] / "implementation/ui"
    module = (root / "src/lib/viewers/colormaps.js").read_text(encoding="utf-8")
    cmaps = root / "node_modules/@niivue/niivue/src/cmaps"

    colours = re.findall(r'hex: "(#[0-9a-f]{6})"', module)
    assert len(colours) >= 12, "a swatch grid is a grid"

    ramps = dict(re.findall(r'\{ name: "([a-z_0-9]+)", group: "([A-Za-z]+)"', module))
    assert len(ramps) >= 20
    assert {"Grayscale", "Heat", "Perceptual", "Clinical"} <= set(ramps.values())

    if not cmaps.exists():
        return  # running from a wheel, with no node_modules beside it

    # Every offered map is one the package actually ships. A name it does not
    # know is a layer that silently keeps the colour it had.
    for name in ramps:
        assert (cmaps / f"{name}.json").exists(), f"NiiVue ships no colormap {name}"

    # And the chip is the ramp, not an impression of it: the stops come from the
    # package's own numbers. Checked on one, because they are all generated by
    # the same tool -- tools/colormaps.mjs, which is what to rerun on upgrade.
    data = json.loads((cmaps / "hot.json").read_text())
    drawn = re.search(r'name: "hot".*?css: "([^"]+)"', module).group(1)
    for at, (red, green, blue) in enumerate(zip(data["R"], data["G"], data["B"])):
        assert f"rgb({red} {green} {blue}) {data['I'][at] / 255 * 100:.1f}%" in drawn
...
def test_a_colour_is_recorded_as_hex_and_a_colormap_by_name(tmp_path):
    """All the file needs to know is which of the two it is.

    Turning a colour into a colormap is the viewer's business, because "the GPU
    only draws with colormaps" is a fact about the GPU and not about the program.
    """
    read = doc.styles("#e5484d@0.45, gray, #FFFFFF!off")
    assert [layer["colormap"] for layer in read] == ["#e5484d", "gray", "#FFFFFF"]
    assert read[0]["opacity"] == 0.45
    assert read[2]["on"] is False
    # And back out unchanged: a colour somebody chose is not ours to normalise.
    assert doc.style_text(read) == "#e5484d@0.45, gray@1.00, #FFFFFF@1.00!off"


def test_something_that_is_neither_still_holds_its_place(tmp_path):
    read = doc.styles("#12345, gray")
    assert read[0]["colormap"] is None, "five hex digits is not a colour"
    assert len(read) == 2


def test_the_viewer_makes_a_colormap_out_of_a_colour():
    """NiiVue paints with colormaps and nothing else.

    A single hue ramping up from transparent, which is the shape the package's
    own `red` and `violet` use -- so a chosen colour and a built-in behave the
    same way laid over a scan. Registered once per instance: `addColormap` on
    every frame would rebuild a table sixty times a second while a slider moves.

    Verified in the page: picking Teal wrote `#12a594` into the directive and put
    1854 draw calls through the GPU, with nothing on the console.
    """
    from pathlib import Path

    source = (
        Path(__file__).resolve().parents[2] / "implementation/ui/src/lib/viewers/Volume.svelte"
    ).read_text(encoding="utf-8")

    assert "function colormapFor(name)" in source
    body = source[source.index("function colormapFor(name)") :]
    body = body[: body.index("\n  }")]
    assert 'name.startsWith("#")' in body
    assert "nv.addColormap(key" in body
    assert "A: [0, 255]" in body, "an overlay has to let the scan through where it is zero"
    assert "painted.has(key)" in body, "registered once, not once a frame"
    # And the table goes with the instance that owns it.
    assert "painted = new Set();" in source[source.index("function teardown()") :][:400]
