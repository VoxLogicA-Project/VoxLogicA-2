...
class ExecutionSession:
            ...
            if self._is_literal_operation(operation):
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    try:
                        ...
                        logger.debug("About to invoke primitive %s with args preview=%s", operator_name, preview)
                    except Exception:
                        logger.debug("Failed to preview args for %s", operator_name)
    ...
    # No special-case coercions here; rely on primitives to validate types
        return resolved
