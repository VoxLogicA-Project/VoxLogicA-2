"""Apply one fold combiner to two values -- one step of a fold.

`fold` used to be a loop inside a kernel: it received the whole materialized
sequence and reduced it in Python, holding every element at once for a result
that never needs more than two values in hand.

Four of the eight combiners have primitives of their own (`+ - * /`); `min`,
`max`, `&&` and `||` never did, because nothing but fold needed them. Taking the
combiner as an ATTRIBUTE covers all eight without four more modules, and leaves
the meaning of fold's operator in the one place that already defines it.
"""
...
PRIMITIVE_SPEC = PrimitiveSpec(
    ...
    description="One step of a fold",
)
