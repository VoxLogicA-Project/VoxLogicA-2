...
def _disk_reserve(total: int, free: int, payload: int) -> int:
    """Bytes that must stay free on this volume, given what the tier holds.

    ONE RULE, ONE PLACE. It is computed from HEADROOM -- what the tier already
    holds plus what is still free -- and never from the volume's size. Clamping
    to half the VOLUME measures the wrong thing: on any volume less than half
    empty it swallows the headroom whole, and a 31 GB tmpfs with 15.4 GB free
    reserved 15.3 GB, leaving a ceiling of zero. The store then kept working
    and kept nothing, silently, while the same run on a roomier disk behaved
    differently.

    This rule lived in `_effective_max_bytes` alone and was copied, with the
    old clamp, into the admission path added later on another branch -- so the
    bug outlived its own fix in a second copy, and the merge of the two kept
    both cleanly. Hence a function.
    """
    headroom = payload + free
    return min(max(_DISK_RESERVE_MIN_BYTES, int(total * _DISK_RESERVE_FRACTION)),
               headroom // 2)
    ...
    def _effective_max_bytes(self) -> int:
        ...
        if self._disk_ceiling is None or (now - self._disk_ceiling_at) >= _DISK_PROBE_INTERVAL_S:
            try:
                ...
                reserve = _disk_reserve(usage.total, usage.free, self._payload_bytes)
    ...
    def _disk_free_and_reserve(self) -> tuple[int, int]:
        ...
        if self._disk_free is None or (now - self._disk_free_at) >= _DISK_PROBE_INTERVAL_S:
            try:
                ...
                return -1, 0
            # THE SAME RULE AS EVICTION, and it has to be: this line carried
            # the half-the-volume clamp that `ad55306` fixed, because it was
            # copied from `_effective_max_bytes` before that fix was merged
            # here. The merge kept both copies cleanly and silently, which is
            # how a fixed bug comes back.
            self._disk_reserve = _disk_reserve(usage.total, usage.free,
                                               self._payload_bytes)
