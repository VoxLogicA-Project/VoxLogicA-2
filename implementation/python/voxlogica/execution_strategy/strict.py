...
class StrictExecutionStrategy(ExecutionStrategy):
    ...
    def _evaluate_node(self, prepared: PreparedPlan, node_id: NodeId) -> Any:
        ...
        if node_id in prepared.values:
            self._cache_summary["cached_local"] = int(self._cache_summary.get("cached_local", 0)) + 1
            self._node_events.append({"node_id": node_id, "status": "cached", "cache_source": "runtime-local"})
            return prepared.values[node_id]
        if node_id in prepared.failures:
            raise ValueError(prepared.failures[node_id])

        node = prepared.definition_store.get(node_id) if prepared.definition_store is not None else prepared.plan.nodes[node_id]
        try:
